...
{
    const  db = await client.db(DbName);
    let data = await db.collection('users').find({_id:Mongodb.ObjectId(req.params.id)}).toArray();
...
router.delete('/delete-user/:id', async (req, res)=> {
  ...
  try {
    const  db = await client.db(DbName);
let data = await db.collection('users').deleteOne({_id:Mongodb.ObjectId(req.params.id)});
res.json({
    statusCode: 200,
